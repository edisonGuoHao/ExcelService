...
import com.edison.excel.EasyExcelFactory;
import com.edison.excel.ExcelReader;
...
public class ExcelUtil {
     ...
     */
    public static <T extends BaseRowModel> List<T> readExcel(MultipartFile file, Class<T> rowModel) throws ExcelException {
        ExcelListener excelListener = new ExcelListener();
        ErrorRowList errorRowList = new ErrorRowList();
        ExcelReader reader = getReader(file, excelListener, errorRowList);
        if (reader == null) {
     ...
     * @return Excel 数据 list
     */
    public static <T extends BaseRowModel> List<T> readExcel(MultipartFile file, Class<T> rowModel, int sheetNo) throws ExcelException {
     ...
     * @return Excel 数据 list
     */
    public static <T extends BaseRowModel> List<T> readExcel(MultipartFile file, Class<T> rowModel, int sheetNo, int headLineNum) throws ExcelException {
        ExcelListener excelListener = new ExcelListener();
        ErrorRowList errorRowList = new ErrorRowList();
        ExcelReader reader = getReader(file, excelListener, errorRowList);
        if (reader == null) {
     ...
     */
    public static <T extends BaseRowModel> void writeExcel(HttpServletResponse response, List<T> list, String fileName, String sheetName, ExcelTypeEnum excelTypeEnum,
                                                           Class<T> classType) throws ExcelException {
        if (sheetName == null || "".equals(sheetName)) {
            sheetName = "sheet1";
        }
        if (excelTypeEnum == ExcelTypeEnum.XLSX) {
            ExcelWriter writer = EasyExcelFactory.getWriterWithTempAndHandler(null, getOutputStream(fileName, response, excelTypeEnum), excelTypeEnum, true, new WriterHandler07<>(classType));
    ...
    public static ExcelWriterFactory writeExcelWithSheets(HttpServletResponse response, List<? extends BaseRowModel> list,
                                                          String fileName, String sheetName, Class<BaseRowModel> object, ExcelTypeEnum excelTypeEnum) throws ExcelException {
        ExcelWriterFactory writer = new ExcelWriterFactory(getOutputStream(fileName, response, excelTypeEnum), excelTypeEnum);
        Sheet sheet = new Sheet(1, 0, object);
        sheet.setSheetName(sheetName);
        writer.write(list, sheet);
        return writer;
    }
    ...
    private static OutputStream getOutputStream(String fileName, HttpServletResponse response, ExcelTypeEnum excelTypeEnum) throws ExcelException {
        ...
        try {
            ...
            response.setHeader("Content-Disposition", "attachment; filename=" + java.net.URLEncoder.encode(fileName, "UTF-8") + excelTypeEnum.getValue());
     ...
     */
    private static ExcelReader getReader(MultipartFile excel, ExcelListener excelListener, ErrorRowList errorRowList) throws ExcelException {
        ...
        try {
            ...
            return new ExcelReader(inputStream, errorRowList, excelListener, false);
        } catch (IOException e) {
            //do something
        }
        return null;
