package com.edison.excel.analysis.v07;

import com.edison.excel.annotation.FieldType;
import com.edison.excel.constant.ExcelXmlConstants;
import com.edison.excel.context.AnalysisContext;
import com.edison.excel.event.AnalysisEventRegisterCenter;
import com.edison.excel.event.OneRowAnalysisFinishEvent;
import com.edison.excel.util.PositionUtils;
...
public class XlsxRowHandler extends DefaultHandler {
    ...
    private void startCellValue(String name) {
        if (name.equals(ExcelXmlConstants.CELL_VALUE_TAG) || name.equals(ExcelXmlConstants.CELL_VALUE_TAG_1)) {
    ...
    private void endCellValue(String name) throws SAXException {
        ...
        } else if (ExcelXmlConstants.CELL_VALUE_TAG_1.equals(name)) {
            curRowContent[curCol] = currentCellValue;
        }
    }
    ...
    private void setTotalRowCount(String name, Attributes attributes) {
        if (ExcelXmlConstants.DIMENSION.equals(name)) {
            String d = attributes.getValue(ExcelXmlConstants.DIMENSION_REF);
    ...
    private void endRow(String name) {
        if (name.equals(ExcelXmlConstants.ROW_TAG)) {
